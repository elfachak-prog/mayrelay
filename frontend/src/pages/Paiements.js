...
export default function Paiements({ user }) {
  ...
  return (
    <div>
      <h2 style={{ fontSize: mobile ? 20 : 24, fontWeight: 700, color: C.dark, marginBottom: 4, fontFamily: 'Georgia, serif' }}>Paiements</h2>
      ...
      {/* Résumé financier */}
      <div style={{ display: 'grid', gridTemplateColumns: mobile ? '1fr 1fr' : 'repeat(3, 1fr)', gap: mobile ? 10 : 14, marginBottom: 24 }}>
        <div style={{ background: C.white, borderRadius: 16, padding: mobile ? '16px 14px' : '22px 24px', border: `1px solid ${C.border}` }}>
          <div style={{ fontSize: mobile ? 20 : 28, marginBottom: 8 }}>💶</div>
          <div style={{ fontSize: mobile ? 20 : 28, fontWeight: 700, color: C.dark, fontFamily: 'Georgia, serif' }}>{paiements.reduce((s, p) => s + parseFloat(p.montant_total), 0).toFixed(2)}€</div>
          <div style={{ fontSize: 11, color: '#888', marginTop: 4, fontFamily: 'sans-serif' }}>Total encaissé</div>
        </div>
        <div style={{ background: C.white, borderRadius: 16, padding: mobile ? '16px 14px' : '22px 24px', border: `1px solid ${C.border}` }}>
          <div style={{ fontSize: mobile ? 20 : 28, marginBottom: 8 }}>🏦</div>
          <div style={{ fontSize: mobile ? 20 : 28, fontWeight: 700, color: C.coral, fontFamily: 'Georgia, serif' }}>{totalDu}€</div>
          <div style={{ fontSize: 11, color: '#888', marginTop: 4, fontFamily: 'sans-serif' }}>À reverser</div>
        </div>
        <div style={{ background: C.white, borderRadius: 16, padding: mobile ? '16px 14px' : '22px 24px', border: `1px solid ${C.border}`, gridColumn: mobile ? '1 / -1' : 'auto' }}>
          <div style={{ fontSize: mobile ? 20 : 28, marginBottom: 8 }}>✅</div>
          <div style={{ fontSize: mobile ? 20 : 28, fontWeight: 700, color: C.green, fontFamily: 'Georgia, serif' }}>{paiements.filter(p => p.statut === 'reverse').length}</div>
          <div style={{ fontSize: 11, color: '#888', marginTop: 4, fontFamily: 'sans-serif' }}>Reversements confirmés</div>
        </div>
      </div>
      ...
      {colis.length > 0 && (
        <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, overflow: 'hidden', marginBottom: 20 }}>
          <div style={{ padding: mobile ? '14px 16px' : '18px 24px', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {colis.map((c, i) => (
            <div key={c.id} style={{ padding: mobile ? '12px 16px' : '14px 24px', borderBottom: i < colis.length - 1 ? `1px solid ${C.border}` : 'none' }}>
              {mobile ? (
                <div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                    <div style={{ fontFamily: 'monospace', fontSize: 11, color: C.lagoon, fontWeight: 600 }}>{c.reference}</div>
                    <div style={{ fontSize: 13, fontWeight: 700, color: C.ocean, fontFamily: 'sans-serif' }}>{c.prix}€</div>
                  </div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ fontSize: 13, color: C.dark, fontFamily: 'sans-serif' }}>{c.nom_destinataire}</div>
      ...
      <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, overflow: 'hidden' }}>
        <div style={{ padding: mobile ? '14px 16px' : '18px 24px', borderBottom: `1px solid ${C.border}` }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: C.dark, fontFamily: 'Georgia, serif' }}>Historique des paiements</div>
        </div>
        {mobile ? (
          <div>
            {paiements.map((p, i) => (
              <div key={p.id} style={{ padding: '12px 16px', borderBottom: i < paiements.length - 1 ? `1px solid ${C.border}` : 'none' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 4 }}>
                  <div style={{ fontFamily: 'monospace', fontSize: 11, color: C.lagoon, fontWeight: 600 }}>{p.reference}</div>
                  <span style={{ background: p.statut === 'reverse' ? '#D1FAE5' : '#FEF3C7', color: p.statut === 'reverse' ? C.green : C.amber, padding: '3px 8px', borderRadius: 20, fontSize: 10, fontWeight: 700, fontFamily: 'sans-serif' }}>
                    {p.statut === 'reverse' ? 'Reversé' : 'En attente'}
                  </span>
                </div>
                <div style={{ fontSize: 13, fontWeight: 600, color: C.dark, fontFamily: 'sans-serif', marginBottom: 4 }}>{p.nom_destinataire}</div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                  <div style={{ fontSize: 11, color: '#AAA', fontFamily: 'sans-serif' }}>{new Date(p.created_at).toLocaleDateString('fr-FR')}</div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: C.green, fontFamily: 'sans-serif' }}>{p.part_partenaire_exp}€ <span style={{ color: '#AAA', fontWeight: 400 }}>/ {p.montant_total}€</span></div>
                </div>
              </div>
            ))}
            {paiements.length === 0 && <div style={{ padding: 32, textAlign: 'center', color: '#888', fontFamily: 'sans-serif' }}>Aucun paiement enregistré</div>}
          </div>
        ) : (
          <>
      ...
      {/* Modal encaissement */}
      {modal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100, padding: mobile ? 16 : 0 }}>
          <div style={{ background: C.white, borderRadius: 20, padding: mobile ? 24 : 36, width: mobile ? '100%' : 440, maxWidth: 440, boxShadow: '0 24px 64px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <div style={{ fontSize: mobile ? 16 : 18, fontWeight: 700, color: C.dark, fontFamily: 'Georgia, serif' }}>Encaisser le paiement</div>
            ...
            <div style={{ marginBottom: 20 }}>
              <div style={{ fontSize: 12, color: C.mid, marginBottom: 10, fontFamily: 'sans-serif', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1.5 }}>Livré par un livreur ?</div>
              <div style={{ display: 'flex', gap: 10 }}>
                {[true, false].map(v => (
                  <div key={v} onClick={() => setAvecLivreur(v)} style={{ flex: 1, padding: mobile ? 10 : 12, border: `2px solid ${avecLivreur === v ? C.lagoon : C.border}`, borderRadius: 10, cursor: 'pointer', textAlign: 'center', fontSize: mobile ? 12 : 13, color: avecLivreur === v ? C.lagoon : '#888', background: avecLivreur === v ? C.foam : C.white, fontWeight: avecLivreur === v ? 700 : 400, fontFamily: 'sans-serif' }}>
                    {v ? '🛵 Oui' : '🏪 Non'}
                  </div>
                ))}
              </div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={handlePayer} disabled={chargement} style={{ flex: 2, padding: 12, background: C.coral, border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'sans-serif', color: C.white }}>
