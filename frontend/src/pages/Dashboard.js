...
export default function Dashboard({ user, onLogout, ongletInitial }) {
  ...
  const [pays1, setPays1] = useState('+262');
  const [pays2, setPays2] = useState('+262');
  ...
  const handleEnvoi = async () => {
    ...
    setChargement(true);
    try {
      const tel1 = pays1 + form.telephone_destinataire.replace(/^0/, '');
      const tel2 = form.telephone2_destinataire ? pays2 + form.telephone2_destinataire.replace(/^0/, '') : '';
      const res = await creerColis({ ...form, telephone_destinataire: tel1, telephone2_destinataire: tel2 });
      ...
      {onglet === 'dashboard' && (
        <div>
          ...
          <div style={{ background: COLORS.white, borderRadius: 16, border: `1px solid ${COLORS.border}`, overflow: 'hidden' }}>
            <div style={{ padding: '18px 24px', borderBottom: `1px solid ${COLORS.border}`, fontSize: 16, fontWeight: 700, color: COLORS.dark }}>Derniers colis</div>
            ...
            <div style={{ background: COLORS.white, borderRadius: 20, padding: 32, border: `1px solid ${COLORS.border}` }}>
              ...
              <div style={{ marginTop: 16 }}>
                ...
                <div style={{ display: 'flex' }}>
                  <SelecteurPays value={pays1} onChange={setPays1} />
              ...
              <div style={{ marginTop: 16 }}>
                <label style={{ display: 'block', fontSize: 11, color: COLORS.mid, marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1.5 }}>Telephone secondaire (optionnel)</label>
                <div style={{ display: 'flex' }}>
                  <SelecteurPays value={pays2} onChange={setPays2} />
              ...
              <div style={{ marginTop: 16 }}>
                <label style={{ display: 'block', fontSize: 11, color: COLORS.mid, marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1.5 }}>Email (optionnel)</label>
              ...
              <div style={{ marginTop: 16 }}>
                <label style={{ display: 'block', fontSize: 11, color: COLORS.mid, marginBottom: 6, textTransform: 'uppercase', letterSpacing: 1.5 }}>Notes (optionnel)</label>
